...
- `skills/dajare-generator/SKILL.md` はルートのコピー（Claude Code プラグイン用）
- SKILL.md を編集するときは **必ずルートの `SKILL.md` だけを編集する**
- `skills/dajare-generator/SKILL.md` は直接編集しない
...
| `skills/dajare-generator/SKILL.md` | `/plugin install` 用コピー | **編集禁止**（hook が自動同期） |
...
- `npx skills add coji/dajare-generator` — ルートの SKILL.md を使用
- `npx openskills install coji/dajare-generator` — AGENTS.md を使用
- `/plugin marketplace add coji/dajare-generator` → `/plugin install dajare-generator@dajare-generator` — .claude-plugin/ + skills/ を使用
