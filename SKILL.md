...
**B. 逆引き韻辞書で網羅検索**（サブエージェントまたは直接実行）：`scripts/rhyme.py --search <元ネタ>` を実行し、母音列が一致・類似する単語を辞書（約4万語）から検索する。LLM の語彙だけでは思いつかない候補が見つかる。サブエージェント（Agent ツール等）が使える環境では、この検索をサブエージェントに委譲して A と並列に走らせること。使えない場合は直接実行する。
...
生成したダジャレの検証は**サブエージェントに委譲する**こと。自分で作ったものを自分で評価すると甘くなりやすい。

サブエージェントが使える環境では、以下の指示で検証エージェントを起動する：

> ダジャレの品質を検証してください。各ダジャレについて以下を順番にチェックし、結果を返してください。
> 1. **音の照合** — `uv run {baseDir}/scripts/rhyme.py <言葉A> <言葉B>` を実行し、母音列の類似度50%未満なら不合格。子音も確認し、掛かり部分の子音が大きく異なる場合も不合格。
> 2. **意味の距離** — 同カテゴリの言葉同士（食べ物同士、動物同士など）なら不合格。
> 3. **文の自然さ** — ダジャレ部分を隠しても日本語として成立するか。
> 4. **失敗パターン照合** — `references/bad-examples.md` の5つの失敗パターンに該当しないか。
> 5. **既存チェック** — 有名すぎる既存ダジャレの焼き直しでないか。
> 各ダジャレについて「合格/不合格」と理由を返してください。

サブエージェントが使えない環境では、上記を自分で実行する。**特に音の照合は必ず `scripts/rhyme.py` を実行**し、LLM の内部判断だけで「合っている」と判定しないこと。

**改善ループ：** 不合格が出て候補が3個未満になった場合、不合格の理由を踏まえてステップ2-3に戻り追加候補を生成する。ただし改善は1回まで。それでも足りなければ、少数でも質の高いものだけを出力する（質 > 数）。
