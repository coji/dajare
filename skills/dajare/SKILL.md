...
同音異義語・子音交換・母音列一致・部分一致・埋め込みなど、あらゆる変換パターンで候補を探す。このとき意味の距離を意識し、元ネタとかけ離れた分野の言葉を積極的に狙う。意味が遠いほど面白くなるため（`references/humor-theory.md` の第2層を参照）。

**逆引き韻辞書の活用：** `scripts/rhyme.py --search <元ネタ>` を実行すると、母音列が一致・類似する単語を辞書（約4万語）から網羅的に検索できる。LLM の語彙だけでは思いつかない候補が見つかるので、積極的に使うこと。
...
以下を順番に検証し、不合格なら容赦なく捨てる。3個しか残らなくてもいい。

1. **音の照合** — 母音列を書き出して一致率50%未満なら不合格。`scripts/rhyme.py` で確認。
2. **意味の距離** — 同カテゴリの言葉同士なら再考。
3. **文の自然さ** — ダジャレ部分を隠しても日本語として成立するか。
4. **既存チェック** — 有名すぎる既存ダジャレの焼き直しでないか。
5. **パターン重複** — 他のダジャレと掛かりの手法が被っていないか。
